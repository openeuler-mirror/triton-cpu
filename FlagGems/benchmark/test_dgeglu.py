...
class DgegluBenchmark(TexGluBackwardBenchmark):
...
@pytest.mark.skipif(flag_gems.device !="cpu" and TE_OP is None, reason="'dgeglu' not found in TransformerEngine")
@pytest.mark.skipif(GEMS_OP is None, reason="'dgeglu' not found in FlagGems")
def test_dgeglu():
    if flag_gems.device =="cpu":
        TE_OP = GEMS_OP
    bench = DgegluBenchmark(
        op_name="dgeglu",
        torch_op=TE_OP,
        gems_op=GEMS_OP,
        dtypes=FLOAT_DTYPES,
