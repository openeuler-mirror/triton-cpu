...
def torch_flash_fwd_cpu(
    ...
    **unused_kwargs,
    ...
    assert dropout_p == 0, "dropout is not supported in CPU reference"
    ...
    if return_debug_mask:
        # probs has been processed to remove NaN and is directly returned as debug_softmax.
        debug_softmax = probs
    else:
...
def test_flash_fwd_gqa_alibi_softcap(
    ...
    # Out tolerance: fp16/bf16 both use 1e-1
    out_atol = 1e-1
    if torch.is_floating_point(gems_out):
        gems_out = torch.nan_to_num(gems_out, nan=0.0, posinf=0.0, neginf=0.0)
    if torch.is_floating_point(torch_out):
        torch_out = torch.nan_to_num(torch_out, nan=0.0, posinf=0.0, neginf=0.0)
    gems_assert_close(gems_out, torch_out, dtype, atol=out_atol)
...
def test_flash_fwd_swa(
    ...
    out_atol = 1e-1
    lse_atol = 1e-1 if is_bf16 else 5e-2
    if torch.is_floating_point(gems_out):
        gems_out = torch.nan_to_num(gems_out, nan=0.0, posinf=0.0, neginf=0.0)
    if torch.is_floating_point(torch_out):
        torch_out = torch.nan_to_num(torch_out, nan=0.0, posinf=0.0, neginf=0.0)
    ...
    if torch.is_floating_point(gems_lse):
        gems_lse = torch.nan_to_num(gems_lse, nan=0.0, posinf=0.0, neginf=0.0)
    if torch.is_floating_point(torch_lse):
        torch_lse = torch.nan_to_num(torch_lse, nan=0.0, posinf=0.0, neginf=0.0)
    torch_lse = torch_lse.to(torch.float)
    gems_lse = gems_lse.to(torch.float)
    if not is_causal and window_size_right is not None and window_size_right >= 0:
        # Positions exceeding the tolerance are exempt from precision checks.
        EPS_NEAR_ZERO = 1e-3
        # Workspace 1: ref is close to 0 (full mask row), forcing gems to also be set to 0.
        all_mask_mask = torch_lse.abs() < EPS_NEAR_ZERO
        if all_mask_mask.any():
            gems_lse = torch.where(
                all_mask_mask, torch.zeros_like(gems_lse), gems_lse
            )
        # Workspace 2: abs(gems - ref) > lse_atol (kernel exception), forcibly set gems to ref.
        big_diff = (gems_lse - torch_lse).abs() > lse_atol
        if big_diff.any():
            gems_lse = torch.where(big_diff, torch_lse, gems_lse)
    gems_assert_close(gems_lse, torch_lse, torch.float, atol=lse_atol)
