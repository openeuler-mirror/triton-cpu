...
def test_flash_fwd_nonsquare_qk(
    ...
    gems_assert_close(gems_out, torch_out, dtype, atol=1e-2)
    ...
    gems_assert_close(gems_lse, torch_lse, torch.float, atol=1e-2)
...
def test_flash_fwd_gqa_alibi_softcap(
    ...
    if torch.is_floating_point(torch_out):
        torch_out = torch.nan_to_num(torch_out, nan=0.0, posinf=0.0, neginf=0.0)
    gems_assert_close(gems_out, torch_out, dtype, atol=5e-2)
